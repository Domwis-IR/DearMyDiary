...
def find_best_Ssymptom(input):
    sentences = slice_sentence(input)
    symptoms = {0: "절망감", 1:"지속되는 우울한 기분", 2:"흥미나 즐거움의 감소", 3: "식욕-체중 불균형", 4: "수면장애", 
                5: "초조나 지체", 6:"피로나 활력상실", 7: "무가치하거나 죄책감", 8: "사고력,집중력감소 또는 우유부단함", 9:"자존감 저하"}
    symptom_count = [0]*9
    for sentence in sentences:
        symptom = symptom_detect(sentence)
        symptom_count[int(symptom[0])] += 1 
    
    m = max(symptom_count)
    a = [i for i,v in enumerate(symptom_count) if v == m]
    result = [symptoms[i] for i in a]
    return result
